...
- `GET /` (document navigation, `Accept` prefers HTML) with `?bill=` — Worker fetches the SPA `index.html` from ASSETS and rewrites `og:title` / `og:description` / `og:url` plus matching Twitter tags from the bill digest (canonical URL `https://trackcongress.org/?bill=...`). Missing bill/digest keeps the static site meta. Not a `/bill/:id` page.
