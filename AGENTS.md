...
- `GET /` (document navigation, `Accept` prefers HTML) with `?bill=` — Worker fetches the SPA `index.html` from ASSETS and rewrites `og:title` / `og:description` / `og:url` plus matching Twitter tags from the bill digest (canonical URL `https://trackcongress.org/?bill=...`). Requires `[assets] run_worker_first = true` so asset-first routing does not serve the static shell before the rewrite (a path include-list disables SPA `Sec-Fetch-Mode` routing and would HTML-fallback `/feed/*.json`). Missing bill/digest keeps the static site meta. Not a `/bill/:id` page.
- `POST /chat/bill` — public UI-message stream (AI SDK `text-*` / `data-quote` / `data-answer`; no `source-document` parts — citations render inline from `data-quote`). Body `{ messages, bill, selection? }` plus ignored `useChat` fields. Grounded on digest + CRS + stored bill-text sections. Burst cap `CHAT_RATE_LIMITER` (10/min per IP, fail-open); daily caps in D1 `chat_usage`, claimed **before** the evidence read so a capped client costs one upsert per 429 (`CHAT_DAILY_PER_CLIENT_CAP` / `CHAT_DAILY_GLOBAL_CAP`; a turn rejected by the per-client cap does **not** touch the site-wide row, and a turn the site-wide cap rejects releases the client slot, so one capped client cannot exhaust everyone's day; `reserveChatUsage` uses a conditional upsert with `RETURNING`). `CHAT_HMAC_SECRET` signs the final answer (`data-answer.sig`) over `bill-chat-answer\n<billParam>\n<text>` so a signature is bound to the bill it was asked about; unset → `sig: null`. Errors: `bad_request` (400), `bill_not_found` (404), `rate_limited` / `daily_limit` (429), `chat_unavailable` (503, missing `OPENROUTER_API_KEY`). Model: `resolveOpenRouterModel` pick first, then the curated `CHAT_FALLBACK_MODELS` via OpenRouter `models` routing (never `openrouter/free` — it routes to classifiers and reasoning-only models), `reasoning: { effort: "low", exclude: true }`, `CHAT_MAX_OUTPUT_TOKENS` 1600; the request `AbortSignal` is forwarded so Stop cancels the model call; a stream that ends inside a `<quote>` tag still verifies the partial passage and logs `bill_chat_truncated`. Provider failures surface as a UI `error` part (`BILL_CHAT_STREAM_ERROR_TEXT` in `shared/chat-api-types.ts`; `useChat` hands it over as `error.message` and `parseChatErrorMessage` shows it verbatim, JSON `{ message }` bodies for 4xx/5xx, else `BILL_CHAT_GENERIC_ERROR_TEXT`), never a blank signed answer. Quote tags are verified by `findChunkForQuote`, which is `findQuoteSource` + `findQuoteInText` from `shared/quote-verification.ts` over the selected evidence chunks (so any passage the chat cites is one `POST /share/quote` accepts); the `section` attribute is a hint that picks the matching chunk first when several contain the passage, and all chunks are still searched when it is missing or wrong. Allowed on preview hosts (not a pipeline write); the preview Worker needs `OPENROUTER_API_KEY` + `CHAT_HMAC_SECRET` set with `wrangler versions secret put <NAME> --env preview` (plain `wrangler secret put --env preview` fails because that Worker only has versions, no deployment). Production needs `CHAT_HMAC_SECRET` added as a secret before answer sharing works. Shared types: `shared/chat-api-types.ts`
- `POST /share/quote` — public, rate limited (`SHARE_RATE_LIMITER`, 20/min per IP; fails **closed** with 503 `rate_limited` if the binding throws, since it is the only per-client bound on this write path) plus a hard `BILL_QUOTES_PER_BILL_CAP` (500 distinct quotes per bill → 429). Body `{ bill: "119-hr-1", text, answer? }`; `text` (12–280 chars, whitespace-collapsed) must be a verbatim substring of a selectable region — digest "what it does" + key points, the CRS summary, or stored bill-text sections (`source: digest|crs|bill_text`; the same `[data-quotable]` regions the UI renders). With `answer: { text, sig }`, `CHAT_HMAC_SECRET` must be set (else `unsupported_source`) and `sig` must verify for **this** bill (else `bad_request`; a signature from another bill's chat is rejected); the quote must be a normalized substring of `answer.text` as signed or of its plain-text fold (`shared/markdown-plain-text.ts` `markdownToPlainText`, applied to the signed side only, so a selection of rendered prose still matches the Markdown the worker signed) and is stored as `source: "answer"` (model prose about the bill, not bill text). Stores `bill_quotes` under a content-derived id (same bill + same text → same id) and returns `{ quote, url }`; `url` is `https://trackcongress.org/?bill=…&quote=<id>` on production hosts and the request origin on preview/local (`publicShareOrigin`). Errors: `quote_too_short|quote_too_long|bad_request|unsupported_source` (400), `bill_not_found` (404), `quote_not_in_bill` (422), `rate_limited` (429/503). Allowed on preview hosts (not a pipeline write). Shared types: `shared/share-api-types.ts`; one matcher for verify + highlight + chat quotes: `shared/quote-verification.ts` (`findQuoteSource` / `findQuoteInText` share the same punctuation classes; a lockstep test pins that anything the API accepts the landing page can mark). Public JSON helpers (body cap, error envelope, IP key) live in `http/public-json.ts`
- `GET /share/quote.json?id=` — one stored quote (`{ quote }`), used by the `?bill=&quote=` landing highlight
- `GET /og/bill/:billParam.png?quote=&v=` — 1200×630 share card PNG (headline or the `quote` text, status line, party-split tally bar) rendered with `workers-og` (satori + resvg WASM) and cached in the Cache API under a key built from bill + quote id + the **server-computed** content version (`ogImageCacheKey`; the request query string never reaches the key, so junk params cannot force re-renders); falls back to the static `/og-image.png` when the bill/quote is unknown or rendering fails (`x-og-card: rendered|fallback`). `/?bill=` document navigations point `og:image` / `twitter:image` here with `v` = content version (`shared/og-card.ts`); with `?quote=` on the page URL, `og:description` / `og:image:alt` become the quote. Worker (`og-card-model.ts`) and web (`utils/ogCardModel.ts`) both feed `assembleOgCardModel` in `shared/og-card.ts`, and both renderers read `OG_CARD_COLORS`, so the share-sheet preview and the PNG cannot drift
...
- `web/src/components/FeedRowDetail.tsx` — expanded feed row detail panel (quotable regions carry `data-quotable`; selecting text opens `SelectionMenu` → `POST /share/quote` or "Ask about this"). Share-sheet state lives in `hooks/useBillShare.ts`; `?bill=&quote=` landing (resolve → highlight in place / callout → scroll + toast) lives in `hooks/useSharedQuoteLanding.ts`
- `web/src/components/BillChatSection.tsx` — grounded “Ask about this bill” chat (`useChat` → `POST /chat/bill`): starters, composer, selection chip, the `Open in ▾` menu (`BillChatExportMenu.tsx`: AI Elements `OpenIn` → ChatGPT / Claude with the plain-text briefing from `utils/billChatExport.ts`, plus `Copy briefing`), and its own `SelectionMenu` for answer-prose selections (signed share via `data-answer.sig`; the detail panel's menu only handles `digest` / `crs` regions)
- `web/src/components/BillChatTranscript.tsx` — message bubbles, inline `QuotedPassage` citations, streaming indicator
- `web/src/components/BillChatLayout.tsx` — one chat surface per page: every mounted `FeedRowDetail` registers as a source via `usePresentBillChat` (returns its "Ask about this" action); `useBillChatSession()` yields the newest source's `BillChatSectionProps`. `routes/Home.tsx` renders it in `#bill-chat-rail` on desktop and `BillChatDrawer.tsx` (vaul peek / half / full; `hooks/useBillChatDrawerSnapHeight.ts` sizes the visible slice) on mobile
...
- `web/src/components/ai-elements/` — vendored Vercel AI Elements (`Conversation`, `Message`/`MessageResponse` (Streamdown), `PromptInput`, `Suggestion`, `OpenIn`, `Loader`), kept verbatim from the registry so they can be refreshed with `npx shadcn@latest add https://registry.ai-sdk.dev/<name>.json` from `web/`. Only local edits: React 18 `forwardRef` shims on `PromptInputTextarea` / `PromptInputButton` (and `ui/input-group.tsx` `InputGroupButton` / `InputGroupTextarea`), because the registry targets React 19 ref-as-prop and Radix `asChild` triggers need the ref; `ui/button.tsx` carries the current registry's `icon-sm` / `icon-lg` sizes that AI Elements expect. Style them through Tailwind tokens, not by restyling their markup: `secondary` is the shadcn fill (`--twc-surface-secondary`) and mid-gray body text is `text-muted-foreground` (`--twc-secondary`); there is no `text-secondary` utility. `styles/bill-chat.css` only lays out the dock/drawer.
