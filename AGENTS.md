...
- `POST /chat/bill` — public UI-message stream (AI SDK `text-*` / `source-document` / `data-quote` / `data-answer`). Body `{ messages, bill, selection? }` plus ignored `useChat` fields. Grounded on digest + CRS + stored bill-text sections. Burst cap `CHAT_RATE_LIMITER` (10/min per IP, fail-open); daily caps in D1 `chat_usage` (`CHAT_DAILY_PER_CLIENT_CAP` / `CHAT_DAILY_GLOBAL_CAP`). `CHAT_HMAC_SECRET` signs the final answer (`data-answer.sig`); unset → `sig: null`. Errors: `bad_request` (400), `bill_not_found` (404), `rate_limited` / `daily_limit` (429), `chat_unavailable` (503, missing `OPENROUTER_API_KEY`). Model: `resolveOpenRouterModel` pick first, then the curated `CHAT_FALLBACK_MODELS` via OpenRouter `models` routing (never `openrouter/free` — it routes to classifiers and reasoning-only models), `reasoning: { effort: "low", exclude: true }`, `CHAT_MAX_OUTPUT_TOKENS` 1600; a stream that ends inside a `<quote>` tag still verifies the partial passage and logs `bill_chat_truncated`. Provider failures surface as a UI `error` part, never a blank signed answer. Allowed on preview hosts (not a pipeline write); the preview Worker needs `OPENROUTER_API_KEY` + `CHAT_HMAC_SECRET` set with `wrangler versions secret put <NAME> --env preview` (plain `wrangler secret put --env preview` fails because that Worker only has versions, no deployment). Production needs `CHAT_HMAC_SECRET` added as a secret before answer sharing works. Shared types: `shared/chat-api-types.ts`
- `POST /share/quote` — public, rate limited (`SHARE_RATE_LIMITER`, 20/min per IP). Body `{ bill: "119-hr-1", text, answer? }`; `text` (12–280 chars, whitespace-collapsed) must be a verbatim substring of the digest, CRS summary, or stored bill-text sections (`source: digest|crs|bill_text`). With `answer: { text, sig }`, `CHAT_HMAC_SECRET` must be set (else `unsupported_source`) and `sig` must verify (else `bad_request`); the quote must be a normalized substring of `answer.text` and is stored as `source: "answer"`. Stores `bill_quotes` under a content-derived id (same bill + same text → same id) and returns `{ quote, url }` with `url = https://trackcongress.org/?bill=…&quote=<id>`. Errors: `quote_too_short|quote_too_long|bad_request|unsupported_source` (400), `bill_not_found` (404), `quote_not_in_bill` (422), `rate_limited` (429). Allowed on preview hosts (not a pipeline write). Shared types: `shared/share-api-types.ts`, matcher: `shared/quote-verification.ts`
- `GET /share/quote.json?id=` — one stored quote (`{ quote }`), used by the `?bill=&quote=` landing highlight
- `GET /og/bill/:billParam.png?quote=&v=` — 1200×630 share card PNG (headline or the `quote` text, status line, party-split tally bar) rendered with `workers-og` (satori + resvg WASM) and cached in the Cache API; falls back to the static `/og-image.png` when the bill/quote is unknown or rendering fails (`x-og-card: rendered|fallback`). `/?bill=` document navigations point `og:image` / `twitter:image` here with `v` = content version (`shared/og-card.ts`); with `?quote=` on the page URL, `og:description` / `og:image:alt` become the quote
