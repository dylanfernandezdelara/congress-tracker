...
### Install and setup
- Run `./scripts/cursor-cloud-setup.sh` (`npm ci` in worker and web, Playwright Chromium, creates `workers/senate_data_worker/.dev.vars` from `.dev.vars.example` when missing or empty).
- The setup script bridges environment variables listed in `BRIDGED_VARS` (`scripts/lib/dev-vars.sh`) — e.g. Cursor Cloud Secrets — into `.dev.vars`, since `wrangler dev` does not read `process.env`. Keys previously bridged from env are removed when cleared in the environment; hand-edited lines for keys never bridged are preserved.
