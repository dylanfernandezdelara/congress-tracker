...
- `GET /og/bill/:billParam.png?q=&v=` — 1200×630 share card PNG (headline or the `q` quote, status line, party-split tally bar) rendered with `workers-og` (satori + resvg WASM) and cached in the Cache API; falls back to the static `/og-image.png` when the bill/quote is unknown or rendering fails (`x-og-card: rendered|fallback`). `/?bill=` document navigations point `og:image` / `twitter:image` here with `v` = content version (`shared/og-card.ts`); with `?quote=` on the page URL (the PNG takes it as `q`), `og:description` / `og:image:alt` become the quote
