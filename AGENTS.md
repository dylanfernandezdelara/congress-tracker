...
- `web/src/components/BillChatSection.tsx` — grounded “Ask about this bill” chat (`useChat` → `POST /chat/bill`): starters, composer, selection chip, and its own `SelectionMenu` for answer-prose selections (signed share via `data-answer.sig`; the detail panel's menu only handles `digest` / `crs` regions)
