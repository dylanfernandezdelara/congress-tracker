...
1. `npm test`
2. For `web/` changes: `npm run dev:web` (separate terminal) then `npm run qa:web`. If the change touches sheets or dialogs, also `npm run qa:sheets` (real Chromium and WebKit: close paths, stacking, focus return, filter Escape, placement, enter motion; jsdom cannot see these). For behavior changes in `web/`, also prove the affected feature with the verify skill (`.cursor/skills/verify-congress-tracker/SKILL.md`) and keep evidence under `artifacts/verify/`.
...
- `POST /share/quote` — public, rate limited (`SHARE_RATE_LIMITER`, 20/min per IP; fails **closed** with 503 `rate_limited` if the binding throws, since it is the only per-client bound on this write path) plus a hard `BILL_QUOTES_PER_BILL_CAP` (500 distinct quotes per bill → 429). Body `{ bill: "119-hr-1", text }`; `text` (12–280 chars, whitespace-collapsed) must be a verbatim substring of a selectable region — digest "what it does" + key points, the CRS summary, or stored bill-text sections (`source: digest|crs|bill_text`; the same `[data-quotable]` regions the UI renders). New quotes are never stored as `source: "answer"`; that value stays in `BillQuoteSource` so older quotes keep resolving. Stores `bill_quotes` under a content-derived id (same bill + same text → same id) and returns `{ quote, url }`; `url` is `https://trackcongress.org/?bill=…&quote=<id>` on production hosts and the request origin on preview/local (`publicShareOrigin`). Errors: `quote_too_short|quote_too_long|bad_request` (400), `bill_not_found` (404), `quote_not_in_bill` (422), `rate_limited` (429/503). Allowed on preview hosts (not a pipeline write). Shared types: `shared/share-api-types.ts`; one matcher for verify + highlight: `shared/quote-verification.ts` (`findQuoteSource` / `findQuoteInText` share the same punctuation classes; a lockstep test pins that anything the API accepts the landing page can mark). Public JSON helpers (body cap, error envelope, IP key) live in `http/public-json.ts`
...
- `web/src/components/FeedRowDetail.tsx` — expanded feed row detail panel (quotable regions carry `data-quotable`; selecting text opens `SelectionMenu` → `POST /share/quote`). Share-sheet state lives in `hooks/useBillShare.ts`; `?bill=&quote=` landing (resolve → highlight in place / callout → scroll + toast) lives in `hooks/useSharedQuoteLanding.ts`
