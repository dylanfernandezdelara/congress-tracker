...
- `web/src/components/FeedRowDetail.tsx` — expanded feed row detail panel (quotable regions carry `data-quotable`; selecting text opens `SelectionMenu` → `POST /share/quote`; `?bill=&q=` landing highlights the quote in place)
