...
const REPLAY_ENV = { DATA_SOURCE: "replay", REPLAY_FIXTURE_SET: "canonical" } as const;

describe("harness runtime", () => {
  it("pins the clock to the fixture instant", () => {
    const harness = buildHarnessConfig(REPLAY_ENV);
  ...
  it("resolves fixture URLs even when api_key is present", () => {
    const fixtureHttp = createFixtureHttp(buildHarnessConfig(REPLAY_ENV));
  ...
  it("serves vote menu XML through the normal fetch helper", async () => {
    const fixture = createFixtureHttp(buildHarnessConfig(REPLAY_ENV));
  ...
  it("is disabled in live mode", () => {
    const fixtureHttp = createFixtureHttp(buildHarnessConfig({ DATA_SOURCE: "live" }));
