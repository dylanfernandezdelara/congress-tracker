...
describe('Home', () => {
  ...
  it('replaces a filtered page before expanding a bill that drops filters', async () => {
    const houseOnly = makeFeedItem({
      bill: { congress: 119, type: 'HR', number: 88, title: 'House Only Act' },
      digest: {
        headline: 'House-only headline',
        what_it_does: 'House only',
        key_points: ['House'],
        terms_explained: [],
      },
      passage_votes: [
    ...
    const senateBill = makeFeedItem({
      bill: { congress: 119, type: 'S', number: 2, title: 'Senate Target' },
      digest: {
        headline: 'Senate origin bill',
        what_it_does: 'Senate target',
        key_points: ['Senate'],
        terms_explained: [],
      },
    })

    let releaseUnfiltered: ((value: ReturnType<typeof pageResponse>) => void) | undefined
    fetchFeed.mockImplementation((options: { chamber?: 'House' | 'Senate'; q?: string; offset?: number }) => {
      if (options.chamber === 'House') {
        return Promise.resolve(pageResponse([houseOnly], { total: 1, has_more: false }))
      }
    ...
    function JumpToBillWithoutFilters() {
    ...
    expect(await screen.findByText('House-only headline')).toBeInTheDocument()
    expect(fetchFeed).toHaveBeenCalledWith({ limit: 15, offset: 0, chamber: 'House' })

    fireEvent.click(screen.getByRole('button', { name: 'Open senate bill' }))

    await waitFor(() => {
      expect(releaseUnfiltered).toBeDefined()
    })
    const callsAfterNavigate = fetchFeed.mock.calls.slice(1)
    expect(callsAfterNavigate).toEqual([[{ limit: 15, offset: 0 }]])

    await act(async () => {
      releaseUnfiltered?.(pageResponse([senateBill], { total: 1, has_more: false }))
    })

    const toggle = await screen.findByRole('button', { name: /Senate origin bill/i })
    await waitFor(() => {
      expect(toggle).toHaveAttribute('aria-expanded', 'true')
    })
    expect(screen.queryByText('House-only headline')).not.toBeInTheDocument()
    expect(fetchFeed.mock.calls.slice(1)).toEqual([[{ limit: 15, offset: 0 }]])
  })
