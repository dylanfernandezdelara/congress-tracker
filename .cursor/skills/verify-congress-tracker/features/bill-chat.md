...
- `chat-section` renders region `Ask about this bill` next to the bill on desktop (`#bill-chat-rail` / complementary `Ask about this bill`) or in the mobile drawer, with an empty-state heading `Ask about H.R. 1` in the log slot, starter chips directly above the composer, and a textbox `Ask about this bill` plus a `Send` button.
