...
- `chat-section` renders region `Ask about this bill` next to the bill on desktop (`#bill-chat-rail` / complementary `Ask about this bill`) or in the mobile drawer, with an empty-state title `Ask about H.R. 1` (`.bill-chat-empty-title`) in the log slot, starter chips directly above the composer, and a textbox `Ask about this bill` plus a `Send` button.
...
- Desktop chat lives in `#bill-chat-rail`, not at the bottom of `Details for <topic>`. Isolated component tests wrap `FeedRowDetail` with `BillChatLayoutProvider` and `<BillChatPane />`. The rail fills the column (`100vh - --site-rail-top - --site-rail-bottom`, 720px on an 800px viewport); the log slot shows the empty state until the first turn, chips sit on a 2-column grid just above the composer, and the composer footer is `Open in ▾ | Send`.
