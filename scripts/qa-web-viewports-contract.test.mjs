...
test('qa:web covers home across the viewport/theme matrix', () => {
  ...
  assert.match(qa, /collectTightnessBars/)
  assert.match(qa, /tightness bar is x-shifted/)
  assert.match(qa, /steamroll 421–1 appeared in tightness bars/)
  assert.doesNotMatch(qa, /collectTightnessDotsInTrack/)
  assert.doesNotMatch(qa, /tightness dots stack on top of each other/)
  assert.match(qa, /IPHONE_SAFARI_UA/)
  assert.match(qa, /Senate tightness row/)
  assert.match(qa, /House tightness row/)
  assert.match(qa, /Senate-waiting list/)
  assert.match(qa, /mobile-narrow/)
  assert.match(qa, /width: 320/)
  assert.match(qa, /width: 390/)
})
