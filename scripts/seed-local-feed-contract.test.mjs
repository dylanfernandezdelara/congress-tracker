...
test('SEED_PRINT_SQL emits schema and idempotent inserts without running wrangler', () => {
  ...
  assert.match(sql, /idx_bill_sponsors_member/)
  ...
  assert.match(sql, /'hr', 99, 'LOCAL:H001'/)
  ...
  assert.match(sql, /CREATE TABLE IF NOT EXISTS bill_text_documents/)
  assert.match(sql, /CREATE TABLE IF NOT EXISTS bill_text_sections/)
